...
The sweep is read-only. Acting on it is not. Three tiers:

**Safe to do without asking** — nothing. There is no fully automatic write in this
agent, by design. The one candidate, processing timesheets with `useDefaultRate: true`,
is reversible and rate-safe, but it books cost against a job and therefore changes
margin. Propose it, batch it, get a yes.

**Propose, then act on approval:**
- Processing timesheets (`useDefaultRate: true` — never a hand-typed rate).
- Allocating a bill whose `items[].job` is populated and whose costing category is
  unambiguous.
- Raising a progress claim for approved variations.
...
both. When the target isn't obvious from supplier, reference, date and site — ask.
...
Runs daily at 7am Brisbane, ahead of the day being committed. Covers **active jobs**
in full — completed jobs don't change day to day and are handled by the backlog review
in `reports/bill-backlog-progress.md`.
